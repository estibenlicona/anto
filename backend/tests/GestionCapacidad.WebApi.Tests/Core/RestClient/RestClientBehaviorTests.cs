...
using GestionCapacidad.RestClient.Handlers;
...
using GestionCapacidad.RestClient.Services;
using Polly.CircuitBreaker;
using Polly.Timeout;
using Xunit;
using AzureAccessToken = Azure.Core.AccessToken;
using TokenCredential = Azure.Core.TokenCredential;
using TokenRequestContext = Azure.Core.TokenRequestContext;

namespace GestionCapacidad.WebApi.Tests.Core.RestClient;
...
public sealed class RestClientBehaviorTests
{
    ...
    public async Task TypedClientRegistration_InjectsRestClientIntoImplementationConstructor()
    {
        ...
        using ServiceProvider provider = BuildClientServices(
            "OrdersApi",
            handler,
        ...
        ITestOrdersApiClient client = provider.GetRequiredService<ITestOrdersApiClient>();

        TestOrderDto response = await client.GetOrderAsync("123");
    ...
    public async Task AccessTokenProvider_RequestsTokenOnlyOnceWhileValid()
    {
        ...
        DefaultAccessTokenProvider provider = CreateTokenProvider(handler);
        AuthOptions options = CreateAuthOptions(scope: "orders.read");

        AccessToken first = await provider.GetTokenAsync("OrdersApi", options, CancellationToken.None);
        AccessToken second = await provider.GetTokenAsync("OrdersApi", options, CancellationToken.None);
    ...
    public async Task OAuthAuthorizationHandler_PostBodyIsPreservedOnFirstSuccessfulRequest()
    {
        ...
        using HttpMessageInvoker invoker = CreateOAuthInvoker(inner, out _);
        using HttpRequestMessage request = CreateJsonPostRequest("""{"name":"first"}""");
    ...
    public async Task OAuthAuthorizationHandler_PostBodyAndContentHeadersArePreservedAcrossUnauthorizedRetry()
    {
        ...
        using HttpMessageInvoker invoker = CreateOAuthInvoker(inner, out FakeAccessTokenProvider? tokenProvider);
        using HttpRequestMessage request = CreateJsonPostRequest("""{"name":"retry"}""");
    ...
    public async Task TypedClient_AppliesDefaultHeaders()
    {
        ...
        Assert.True(handler.Requests[0].Headers.TryGetValues("X-Client", out IEnumerable<string>? values));
    ...
    {
        GestionCapacidad.RestClient.Services.RestClient client = CreateRestClient(new RecordingHandler(_ => new HttpResponseMessage(HttpStatusCode.NoContent)));

        InvalidOperationException exception = await Assert.ThrowsAsync<InvalidOperationException>(() => client.GetAsync<TestOrderDto>("/orders/123"));
    ...
    public async Task PostAsync_WithNoContentThrowsClearInvalidOperationException()
    {
        ...
        InvalidOperationException exception = await Assert.ThrowsAsync<InvalidOperationException>(() => client.PostAsync<TestOrderDto>("/orders", new { Id = "123" }));
    ...
    {
        GestionCapacidad.RestClient.Services.RestClient client = CreateRestClient(new RecordingHandler(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent(string.Empty)
        }));

        InvalidOperationException exception = await Assert.ThrowsAsync<InvalidOperationException>(() => client.PutAsync<TestOrderDto>("/orders/123", new { Id = "123" }));
    ...
    public async Task DeleteAsync_SucceedsWithNoContent()
    {
        ...
        GestionCapacidad.RestClient.Services.RestClient client = CreateRestClient(handler);

        await client.DeleteAsync("/orders/123");

        Assert.Single(handler.Requests);
        Assert.Equal(new Uri("https://orders.example/orders/123"), handler.Requests[0].RequestUri);
    }
    ...
    public async Task TimeoutOnly_DoesNotRetry()
    {
        ...
        ExternalApiException exception = await Assert.ThrowsAsync<ExternalApiException>(
    ...
    public async Task RetryTransient_RetriesInternalServerErrorForGet()
    {
        ...
        TestOrderDto order = await provider.GetRequiredService<ITestOrdersApiClient>().GetOrderAsync("123");
    ...
    public async Task RetryTransient_RetriesHttpRequestExceptionForSafeMethods()
    {
        ...
        {
            attempts++;
            return attempts == 1 ? throw new HttpRequestException("network") : JsonResponse("""{"id":"123","status":"ok"}""");
        });
    ...
    public async Task RetryHandleTimeouts_ExplicitlyEnablesTimeoutRetry()
    {
        ...
        {
            attempts++;
            return attempts == 1 ? throw new TimeoutRejectedException("timeout") : JsonResponse("""{"id":"123","status":"ok"}""");
        });
    ...
    public async Task RetryUnsafeMethods_EnablesPostRetry()
    {
        ...
        TestOrderDto order = await provider.GetRequiredService<ITestOrdersApiClient>().CreateOrderAsync("123");
    ...
    {
        Dictionary<string, string?> configuration = BaseConfiguration("OrdersApi")
            .With("HttpClients:OrdersApi:Resilience:Preset", "TimeoutOnly")
            .With("HttpClients:OrdersApi:Resilience:Retry:Attempts", "1");

        InvalidOperationException exception = Assert.Throws<InvalidOperationException>(() => BuildClientServices("OrdersApi", new RecordingHandler(_ => JsonResponse("{}")), configuration));
    ...
    public async Task MultipleTypedClients_UseIndependentBaseUrlsAuthHeadersAndResiliencePipelines()
    {
        ...
        TestOrderDto order = await provider.GetRequiredService<ITestOrdersApiClient>().GetOrderAsync("o-1");
    ...
    public void Serializer_DeserializesCaseInsensitiveJson()
    {
        ...
        TestOrderDto order = serializer.Deserialize<TestOrderDto>("""{"ID":"123","STATUS":"ok"}""");
    ...
    public async Task RestClient_ThrowsExternalApiExceptionForNonSuccessResponse()
    {
        ...
        ExternalApiException exception = await Assert.ThrowsAsync<ExternalApiException>(() => client.GetAsync<TestOrderDto>("/orders/123"));
    ...
    [Fact]
    public void ManagedIdentityAuth_RequiresScopes()
    {
        Dictionary<string, string?> configuration = BaseConfiguration("OrdersApi")
            .With("HttpClients:OrdersApi:Auth:Type", "ManagedIdentity")
            .With("HttpClients:OrdersApi:Auth:Scopes:0", " ");

        InvalidOperationException exception = Assert.Throws<InvalidOperationException>(() =>
            BuildClientServices("OrdersApi", new RecordingHandler(_ => JsonResponse("{}")), configuration));

        Assert.Contains("Auth:Scopes", exception.Message);
    }

    [Fact]
    public void ManagedIdentityAuth_DoesNotRequireClientSecret()
    {
        Dictionary<string, string?> configuration = BaseConfiguration("OrdersApi")
            .With("HttpClients:OrdersApi:Auth:Type", "ManagedIdentity")
            .With("HttpClients:OrdersApi:Auth:Scopes:0", "api://orders/.default");

        using ServiceProvider provider = BuildClientServices(
            "OrdersApi",
            new RecordingHandler(_ => JsonResponse("""{"id":"123","status":"ok"}""")),
            configuration);
    ...
    [Fact]
    public async Task ManagedIdentityAuth_UsesTokenProvider()
    {
        using var inner = new RecordingHandler(_ => JsonResponse("""{"ok":true}"""));
        var tokenProvider = new FakeManagedIdentityTokenProvider("fake-token");
        using HttpMessageInvoker invoker = CreateManagedIdentityInvoker(inner, tokenProvider);
        using var request = new HttpRequestMessage(HttpMethod.Get, "https://orders.example/orders/1");
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Single(inner.Requests);
        Assert.Equal("Bearer fake-token", inner.Requests[0].Headers.Authorization?.ToString());
        Assert.Equal(new string[] { "api://orders/.default" }, tokenProvider.Scopes);
        Assert.Equal("user-assigned-client-id", tokenProvider.ClientId);
    }

    [Fact]
    public async Task ManagedIdentityAuth_PropagatesCancellationToken()
    {
        using var inner = new RecordingHandler(_ => JsonResponse("""{"ok":true}"""));
        var tokenProvider = new FakeManagedIdentityTokenProvider("fake-token");
        using HttpMessageInvoker invoker = CreateManagedIdentityInvoker(inner, tokenProvider);
        ...
        using HttpResponseMessage response = await invoker.SendAsync(request, cancellationTokenSource.Token);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal(cancellationTokenSource.Token, tokenProvider.CancellationToken);
    }

    [Fact]
    public async Task ManagedIdentityTokenFailure_DoesNotExposeTokenOrSecretInMessageOrLogs()
    {
        using var handler = new RecordingHandler(_ => JsonResponse(TokenJson("unused-token", expiresIn: 3600)));
        var logger = new CapturingLogger<DefaultAccessTokenProvider>();
        var provider = new DefaultAccessTokenProvider(
            new StaticHttpClientFactory(handler),
            new DefaultRestClientSerializer(),
            new FailingManagedIdentityTokenProvider(),
            logger);

        InvalidOperationException exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            provider.GetTokenAsync("OrdersApi", CreateManagedIdentityAuthOptions(), CancellationToken.None));

        Assert.Contains("Managed Identity token acquisition failed for outbound client OrdersApi.", exception.Message);
        Assert.DoesNotContain("fake-token-value", exception.ToString(), StringComparison.Ordinal);
        Assert.DoesNotContain("fake-client-secret", exception.ToString(), StringComparison.Ordinal);
        Assert.Empty(logger.Messages);
    }

    [Fact]
    public async Task ManagedIdentityTokenProvider_UsesSystemAssignedCredentialWhenClientIdIsEmpty()
    {
        var credential = new FakeTokenCredential("system-token");
        var factory = new FakeManagedIdentityCredentialFactory(credential);
        var tokenProvider = new ManagedIdentityTokenProvider(factory);

        AccessToken token = await tokenProvider.GetTokenAsync(
            ["api://orders/.default"],
            string.Empty,
            CancellationToken.None);

        Assert.Equal("system-token", token.Value);
        Assert.Equal(string.Empty, Assert.Single(factory.ClientIds));
    }

    [Fact]
    public async Task ManagedIdentityTokenProvider_UsesUserAssignedCredentialWhenClientIdIsConfigured()
    {
        var credential = new FakeTokenCredential("user-token");
        var factory = new FakeManagedIdentityCredentialFactory(credential);
        var tokenProvider = new ManagedIdentityTokenProvider(factory);
        ...
        Assert.Equal("user-assigned-client-id", Assert.Single(factory.ClientIds));
    }
    ...
    private static HttpMessageInvoker CreateManagedIdentityInvoker(
        RecordingHandler inner,
        IManagedIdentityTokenProvider managedIdentityTokenProvider)
    {
        var tokenProvider = new DefaultAccessTokenProvider(
            new StaticHttpClientFactory(inner),
            new DefaultRestClientSerializer(),
            managedIdentityTokenProvider,
            NullLogger<DefaultAccessTokenProvider>.Instance);

        var handler = new OAuthAuthorizationHandler(
            "OrdersApi",
            CreateManagedIdentityAuthOptions(),
            tokenProvider)
        {
            InnerHandler = inner
        };
    ...
    private static GestionCapacidad.RestClient.Services.RestClient CreateRestClient(HttpMessageHandler handler)
    {
        var httpClient = new HttpClient(handler) { BaseAddress = new Uri("https://orders.example/") };
        return new GestionCapacidad.RestClient.Services.RestClient(
            httpClient,
            new DefaultRestClientSerializer(),
            new DefaultHttpErrorHandler(NullLogger<DefaultHttpErrorHandler>.Instance),
            NullLogger<GestionCapacidad.RestClient.Services.RestClient>.Instance);
    }
    ...
    private static DefaultAccessTokenProvider CreateTokenProvider(HttpMessageHandler handler)
    {
        ...
        return new DefaultAccessTokenProvider(
            factory,
            new DefaultRestClientSerializer(),
            new FakeManagedIdentityTokenProvider("managed-identity-token"),
            NullLogger<DefaultAccessTokenProvider>.Instance);
    }
    ...
    private static AuthOptions CreateManagedIdentityAuthOptions()
        => new()
        {
            Type = AuthenticationType.ManagedIdentity,
    ...
    private sealed class RecordingHandler : HttpMessageHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                HttpResponseMessage response = await _respond(request, cancellationToken).ConfigureAwait(false);
    ...
    private sealed class FakeManagedIdentityTokenProvider(string token) : IManagedIdentityTokenProvider
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            ClientId = clientId;
            CancellationToken = cancellationToken;
            return ValueTask.FromResult(new AccessToken("Bearer", token, DateTimeOffset.UtcNow.AddMinutes(5)));
        }
    }

    private sealed class FailingManagedIdentityTokenProvider : IManagedIdentityTokenProvider
    {
        public ValueTask<AccessToken> GetTokenAsync(
            string[] scopes,
            string? clientId,
            CancellationToken cancellationToken)
            => throw new InvalidOperationException("Credential unavailable for outbound client.");
    }
    ...
    private sealed class FakeManagedIdentityCredentialFactory(TokenCredential credential) : IManagedIdentityCredentialFactory
    {
    ...
    private sealed class FakeTokenCredential(string token) : TokenCredential
    {
        ...
        public override ValueTask<AzureAccessToken> GetTokenAsync(
            TokenRequestContext requestContext,
            CancellationToken cancellationToken)
            => ValueTask.FromResult(new AzureAccessToken(token, DateTimeOffset.UtcNow.AddMinutes(5)));
    }
