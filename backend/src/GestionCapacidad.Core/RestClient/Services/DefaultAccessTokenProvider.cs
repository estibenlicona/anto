    ...
    ILogger<DefaultAccessTokenProvider> logger) : IAccessTokenProvider
{
    ...
    public async Task<AccessToken> GetTokenAsync(string clientName, AuthOptions options, CancellationToken cancellationToken)
    {
        ...
        if (TryGetValidToken(cacheKey, out AccessToken? cachedToken))
        {
            return cachedToken;
        }
        ...
        try
        {
            ...
            AccessToken token = await RequestTokenAsync(clientName, options, cancellationToken).ConfigureAwait(false);
    ...
    {
        if (_tokenCache.TryGetValue(cacheKey, out AccessToken? cachedToken) && DateTimeOffset.UtcNow < cachedToken.ExpiresAt)
        {
            token = cachedToken;
            return true;
        }
    ...
    {
        if (options.Type == AuthenticationType.ManagedIdentity)
        {
            var scopes = NormalizeScopes(options.Scopes);
            try
            {
                return await managedIdentityTokenProvider
                    .GetTokenAsync(scopes, options.ClientId, cancellationToken)
                    .ConfigureAwait(false);
            }
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
                throw new InvalidOperationException(
                    $"Managed Identity token acquisition failed for outbound client {clientName}.",
                    exception);
            }
        }
        ...
        HttpContent? content = CreateTokenContent(options);
        ...
        return string.IsNullOrWhiteSpace(tokenResponse.TokenType) || string.IsNullOrWhiteSpace(tokenResponse.AccessToken)
    ...
    private static void ValidateAuthOptions(string clientName, AuthOptions options)
    {
        ...
        if (options.Type == AuthenticationType.ManagedIdentity)
        {
            if (NormalizeScopes(options.Scopes).Length == 0)
            {
                throw new InvalidOperationException($"Client {clientName} must define at least one Auth:Scopes value.");
            }
    ...
    private static string CreateCacheKey(string clientName, AuthOptions options)
        => string.Join(
            ...
            options.Scope,
            options.Audience,
            string.Join(ScopeDelimiter, NormalizeScopes(options.Scopes)));
    ...
    private static HttpContent? CreateTokenContent(AuthOptions options)
    {
        ...
        Dictionary<string, string> body = CreateTokenRequestBody(options);
