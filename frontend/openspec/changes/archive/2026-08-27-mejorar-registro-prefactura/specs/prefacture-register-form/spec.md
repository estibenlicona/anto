...
### Requirement: La imputación se anuncia como opcional y llega prellenada
La zona "Imputación" SHALL mostrar junto a su título la etiqueta "Opcional" y, debajo, la explicación de que suele llegar después que el documento y que lo que falte queda marcado en el detalle. "Célula o CoE" y "Concepto" SHALL ir a ancho completo; "Cuenta contable", "Número de cuenta contable", "Centro de costos" y "Orden de compra" SHALL compartir filas a dos columnas; "Cuenta destinada al pago" SHALL ir a ancho completo. En un primer registro, "Célula o CoE" SHALL llegar prellenada con la célula de la persona cuando la tiene, y "Concepto" SHALL sugerir "Servicios profesionales" como placeholder sin enviarlo. Los siete datos SHALL seguir siendo opcionales: lo vacío se envía como ausente.
...
- **THEN** "Célula o CoE" muestra "Backend Platform" y "Concepto" muestra el placeholder "Servicios profesionales"
...
La zona "Prefactura" SHALL abrir con la acción "Cargar PDF", que SHALL aceptar únicamente archivos PDF. Con un archivo elegido, el formulario SHALL mostrar su nombre, cambiar la acción a "Cambiar PDF" y ofrecer "Quitar". Sin archivo, SHALL explicar que cuando el PDF se pueda leer llenará número, fecha y valor. Adjuntar el PDF NO SHALL ser obligatorio ni cambiar lo que se envía al registrar: la lectura automática del documento es del backend y no forma parte de este cambio.

#### Scenario: Adjuntar un PDF
- **WHEN** el usuario elige "prefactura-agosto.pdf" desde "Cargar PDF"
- **THEN** el formulario muestra "prefactura-agosto.pdf" junto a "Cambiar PDF" y "Quitar"
- **AND** al pulsar "Quitar" vuelve a mostrar "Cargar PDF" con su explicación
