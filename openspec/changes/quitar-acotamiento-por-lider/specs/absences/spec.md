...
- FROM: `### Requirement: Pantalla de ausencias del chapter`
- TO: `### Requirement: Pantalla de ausencias`

## MODIFIED Requirements

### Requirement: Pantalla de ausencias
El sistema SHALL ofrecer una pantalla de Ausencias en `/app/lead/ausencias` que lista **todas las ausencias registradas** por mes, sin recortar por quién tiene a cargo a cada persona: abre en el mes actual, permite navegar a meses anteriores y siguientes, y muestra las ausencias cuyo rango toca el mes visible. El breadcrumb de la pantalla SHALL ser "Gestionar Ausencias".

La pantalla SHALL encabezar el listado con tres lecturas del mes visible: cuántas ausencias hay y cuántos días hábiles suman, el impacto en FTE de las ausencias aprobadas **frente al FTE total de ese mes** (nombrando la célula que más pierde), y cuántas solicitudes esperan decisión. Cada fila SHALL mostrar la persona (con su proveedor debajo del nombre cuando es de un tercero, o "Planta" cuando no), el tipo como etiqueta categórica, el rango de fechas, los días hábiles, la célula más afectada, el impacto en FTE del mes visible, el estado y las acciones que su estado admite: aprobar y rechazar mientras está Solicitada, y rechazar cuando ya está Aprobada, porque una aprobación equivocada tiene que poder corregirse.
...
El sistema SHALL permitir registrar una ausencia desde la pantalla, eligiendo **cualquier persona registrada**, un tipo — Vacaciones, Permiso o Incapacidad — y un rango de fechas (un solo día es un rango de un día). Los días de la ausencia SHALL contarse como días hábiles (lunes a viernes) del rango. Toda ausencia registrada SHALL nacer en estado Solicitada.
...
La lectura agregada SHALL expresar el impacto **contra el FTE total de ese mes**, no como una cifra suelta: un descuento en FTE no se interpreta sin saber de qué total sale. SHALL nombrar además la célula que más FTE pierde en el mes, diciendo qué significa esa lectura en vez de darla por sabida — quien mira la pantalla por primera vez no tiene por qué deducirlo del texto.
...
- **THEN** ve cuánto FTE descuentan las ausencias aprobadas y sobre qué total se descuenta, de modo que la cifra se entiende sin conocer de antemano cómo se calcula
