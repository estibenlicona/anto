...
El sistema SHALL mostrar, arriba del listado de Personas, un encabezado con el título del módulo, su descripción, el botón para dar de alta una persona, y un resumen de 3 indicadores sobre **todas las personas registradas** (no sobre la página ni el filtro actual del listado): total de personas activas con sus avatares, FTE disponible frente a la capacidad objetivo, y distribución por seniority. Los avatares del resumen SHALL usar el mismo color por persona que el listado.
...
- **WHEN** el sistema registra personas repartidas entre varios responsables y alguien abre la pantalla
- **THEN** los tres indicadores se calculan sobre todas las personas registradas: el recorte por responsable ya no existe, y el título de este escenario describe la regla anterior, no la vigente
...
- **WHEN** el sistema registra personas repartidas entre varios responsables
- **THEN** el listado las contiene a todas: el recorte por responsable ya no existe, y el título de este escenario describe la regla anterior, no la vigente
