...
El sistema SHALL mostrar una navegación lateral con como máximo estas 6 entradas, en este orden y agrupación, tomando de `NAV.admin` del mockup de referencia su estructura y su orden pero no su redacción:
- Inicio
- Grupo "Configuración": Sprints, Parámetros, Habilidades, Líneas
- Grupo "DevOps": Ingesta

Cada entrada salvo "Inicio" SHALL declarar el permiso de sección del módulo que exige — Sprints `Capacidad.Sprints`, Parámetros `Capacidad.Parametros`, Habilidades `Capacidad.Habilidades`, Líneas `Capacidad.Lineas`, Ingesta `Capacidad.DevOps` — y el menú SHALL mostrar únicamente las entradas cuyos permisos la sesión tiene, desapareciendo los grupos que queden vacíos. "Inicio" SHALL mostrarse siempre a quien entró al shell.
...
- **WHEN** el usuario con los permisos de todas las secciones navega a cualquier ruta bajo `/app/admin`
- **THEN** la navegación lateral muestra las 6 entradas agrupadas como arriba

#### Scenario: Sin el permiso de una sección
- **WHEN** la sesión no tiene `Capacidad.DevOps`
- **THEN** la entrada "Ingesta" no se muestra y el grupo "DevOps" desaparece, mientras el grupo "Configuración" queda intacto
