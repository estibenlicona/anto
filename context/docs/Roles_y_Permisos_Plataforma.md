...
| Administrar equipos (agrupan células): crear, editar, eliminar | ● | ● | ● | — | — | ✅ |
...
| Líder Técnico | `Plataforma.TechLead` ✅ | `tech-lead` ✅ | `TechnicalLead` | `/app/tech` ❌ (hoy entra a Gestión de Capacidad, sin acotar por célula) | Células con `Squad.technicalLeadId` = esa persona (campo nuevo) ⚠ R-03 |
...
- **Permisos de sección (resuelto, change `add-capacity-section-permissions`; ampliado por `add-teams-module`).** Las secciones internas de Gestión de Capacidad no se autorizan con scopes sino con **app roles de la app registration de la API del módulo** (`api://capacidad`), que viajan como sub-claims en el claim `roles` del access token dirigido a esa API — permisos **por persona**, cosa que `scp` (por aplicación cliente) no puede expresar. Catálogo: `Capacidad.{Iniciativas,Celulas,Personas,Ausencias,Dedicacion,Prefacturacion,Competencias,Sprints,Parametros,Habilidades,Lineas,Equipos,DevOps}` — uno por entrada del sidebar; «Inicio» no exige permiso. El menú (`filterNav`) y los guards de ruta exigen el mismo permiso desde un mapa compartido por shell. Asignaciones de la semilla local (`pnpm entra:seed`): Administrador los 13; Líder de Expertise los 7 de su shell (suma `Equipos`, y ya no lleva `Iniciativas`: la demanda la registra y evalúa quien la pide, no quien administra las personas); Líder Técnico `Iniciativas`, `Celulas`, `Dedicacion`, `Competencias`, `Equipos`; sin rol, ninguno. Los scopes (`capacidad.read`…) quedan para lo que son: lo que la app cliente puede pedir, no lo que la persona puede ver.
- **El registro de módulos del host ya admite al Líder Técnico en Gestión de Capacidad** (antes sólo Administrador y Líder de Expertise; decisión de `add-entra-local-sign-in`, revertida a pedido explícito). Entra con las secciones de arriba — no tiene todavía un shell propio `/app/tech` acotado a su célula (§5, R-03/R-04): hoy ve, sin acotar por célula, las mismas pantallas de Iniciativas, Células, Dedicación, Competencias y Equipos que el resto de quienes entran a ese módulo.
