...
**Reason**: El requisito se retira para poder retirar con él su escenario "El matiz dorado supera el piso de contraste", que nombra un matiz que deja de existir — el validador no permite que un MODIFIED omita escenarios vigentes, así que la redefinición viaja como baja y alta. El contrato reaparece completo, en su forma turquesa, como "Vocabulario de acento sin significado de estado" en la sección ADDED de este mismo delta.
**Migration**: Ninguna para consumidores de tokens o componentes — cambian los nombres de matiz (`gold` → `teal`), no la forma de consumo. Quien referencie el requisito por su nombre anterior lo encuentra ahora como "Vocabulario de acento sin significado de estado".
...
- **WHEN** se verifica el paso de relleno del matiz `teal` contra las superficies donde el sistema coloca un segmento teñido (fila clara, lienzo, fila seleccionada y fila en tema oscuro)
