...
La navegación y la página de detalle de cada componente SHALL exponer el estado de madurez del componente (por ejemplo, estable o beta), de modo que un usuario distinga qué componentes puede adoptar sin reservas antes de abrir su página.

#### Scenario: Estado visible en la navegación
- **WHEN** un usuario recorre la lista de componentes en el sidebar
- **THEN** los componentes que no están estables muestran junto a su nombre una insignia que lo indica
...
La página de detalle de cada componente SHALL indicar el peso del código que el CLI copiará al proyecto, para que el usuario pueda estimar el costo de adoptarlo.

#### Scenario: Consultar el peso antes de instalar
- **WHEN** un usuario abre la página de detalle de un componente
- **THEN** ve, junto al comando de instalación, el peso del componente
...
El sitio SHALL incluir una página de instalación que documente los requisitos previos del proyecto anfitrión, el comando de inicialización del CLI con su diálogo y sus salidas, el archivo de configuración que genera, y cómo traer el primer componente.

#### Scenario: Instalar tuip en un proyecto nuevo
- **WHEN** un usuario abre la página de instalación
- **THEN** encuentra los requisitos del proyecto, el comando de inicialización y el resultado esperado de ejecutarlo

#### Scenario: Consultar el archivo de configuración
- **WHEN** un usuario abre la página de instalación
- **THEN** ve el contenido del archivo de configuración que el CLI genera y qué significa cada una de sus opciones

### Requirement: Página de referencia del CLI
El sitio SHALL incluir una página que documente cada comando del CLI: qué hace, cómo se invoca y qué salida produce; y que liste las banderas globales con su tipo, su valor por defecto y su efecto.

#### Scenario: Consultar un comando
- **WHEN** un usuario abre la página del CLI buscando cómo agregar un componente
- **THEN** encuentra el comando, su forma de invocación y un ejemplo de su salida

#### Scenario: Consultar las banderas globales
- **WHEN** un usuario abre la página del CLI
- **THEN** ve una tabla de banderas globales con el tipo, el valor por defecto y el efecto de cada una

### Requirement: Página de anatomía de un proyecto
El sitio SHALL incluir una página que muestre dónde queda cada archivo en el proyecto anfitrión después de instalar componentes, y que distinga qué archivos puede editar el equipo y cuáles gestiona el sistema y son sobrescritos al actualizar.

#### Scenario: Ubicar el código copiado
- **WHEN** un usuario abre la página de anatomía de un proyecto
- **THEN** ve el árbol de carpetas resultante de instalar componentes con el CLI

#### Scenario: Saber qué se puede editar
- **WHEN** un usuario abre la página de anatomía de un proyecto
- **THEN** distingue qué archivos puede modificar sin que una actualización los pise y cuáles no

### Requirement: Páginas de fundamentos
El sitio SHALL documentar los fundamentos del sistema en páginas separadas por tema —tipografía, color y tokens, y espaciado—, cada una accesible desde la sección de fundamentos de la navegación. Cada página SHALL presentar los valores del tema junto con el nombre del token que los expresa y la orientación de cuándo aplicar cada uno.
...
La ruta raíz del sitio SHALL mostrar una página de inicio que presente el sistema de diseño e indique cómo instalar y empezar a usar el CLI, con accesos directos a la documentación de los componentes y a los fundamentos del sistema.
...
Cada página de componente SHALL mostrar en su cabecera el comando del CLI necesario para agregarlo a un proyecto React, presentado como un bloque copiable junto a los metadatos del componente.

#### Scenario: Ver comando de instalación
- **WHEN** un usuario abre la página de detalle de un componente
- **THEN** el sitio muestra en la cabecera el comando exacto para agregar ese componente a un proyecto

#### Scenario: Copiar el comando desde la cabecera
- **WHEN** un usuario usa la acción de copiar del comando de instalación
- **THEN** el comando se copia al portapapeles sin abandonar la página

## REMOVED Requirements

### Requirement: Navegación de dos niveles
**Reason**: De los tres niveles de despliegue que definía el requisito, dos no se ganaban su costo. El desplegable por componente listaba sus ejemplos, que el rail "En esta página" ya enumera, de modo que el sidebar mantenía un segundo índice de lo mismo; y el desplegable de las secciones de primer nivel permitía esconder Empezar o Fundamentos, que son listas cortas y fijas. El agrupamiento por categoría dentro de Componentes sí se conserva —es el nivel que evita que el sidebar crezca sin límite con el catálogo—, pero pasa a ser plegable en vez de un encabezado inerte. Todo eso queda definido en el requisito "Navegación del sitio".

**Migration**: Todas las páginas siguen alcanzables desde el sidebar; ninguna URL cambia. Los ejemplos de un componente dejan de listarse en el sidebar y se alcanzan desde el índice "En esta página" del rail derecho, que ya los enumeraba. Las secciones de primer nivel dejan de poder contraerse. Los encabezados de categoría, que antes rotulaban sin ser controles, ahora despliegan y contraen su grupo, y llegan contraídos salvo el de la página actual.

### Requirement: Página de inicio del sitio
**Reason**: El requisito obligaba a que la página de inicio ofreciera un acceso directo al catálogo, que este change retira. Se reemplaza por el requisito "Entrada al sitio", idéntico salvo por el destino de ese acceso.

**Migration**: La ruta raíz no cambia y sigue presentando el sistema y el comando de instalación. El acceso que llevaba al catálogo pasa a llevar a la documentación de un componente, y el segundo acceso apunta a los fundamentos.

### Requirement: Catálogo navegable de componentes
**Reason**: Con los componentes agrupados por categoría y plegables en el sidebar, una página que vuelve a listarlos todos es un segundo índice del mismo conjunto, que hay que mantener en paralelo y que compite con el sidebar por ser el punto de entrada al catálogo. La entrada al sitio la da la página de inicio; el recorrido del catálogo lo da el sidebar.

**Migration**: Se retira la ruta del catálogo. Todos los componentes siguen alcanzables desde la sección de componentes del sidebar y desde la búsqueda, y sus URL de detalle no cambian. El enlace global del header y el acceso de la página de inicio pasan a llevar a la documentación de un componente. Se pierde la vista previa renderizada en vivo que el catálogo mostraba junto a cada nombre: no se reemplaza, y quien quiera ver el componente antes de leerlo lo encuentra en la pestaña Uso de su página.

### Requirement: Código fuente visible y copiable
**Reason**: El requisito ataba el tema del resaltado de sintaxis al tema activo del sitio, que deja de existir al adoptarse un tema único. Se reemplaza por el requisito "Código fuente del componente", idéntico salvo por esa atadura.

**Migration**: El visor de código conserva barra de título, resaltado, numeración de línea y copia al portapapeles. El resaltado usa un único tema de sintaxis en vez de alternar entre uno claro y uno oscuro.

### Requirement: Selector de tema claro/oscuro
**Reason**: El diseño del sitio se define sobre un único tema claro. Sostener dos temas duplicaba las decisiones de color de cada superficie del sitio sin que hubiera demanda de modo oscuro por parte del equipo.

**Migration**: El sitio se muestra siempre en tema claro. Se retiran el control de tema del header y la persistencia de la elección; la preferencia previamente guardada por un usuario deja de tener efecto y no requiere limpieza manual. El modo oscuro de los componentes de Tuya UI no se ve afectado: sigue existiendo para las aplicaciones que lo consumen, solo deja de usarlo el sitio de documentación.

### Requirement: Página de design tokens
**Reason**: Una sola página no permitía dar a tipografía, color y espaciado el desarrollo que cada tema necesita —escala completa, orientación de uso y reglas de aplicación—, y el índice de la página resultaba demasiado largo para orientarse.

**Migration**: El contenido se reparte entre las tres páginas de la sección de fundamentos definidas en el requisito "Páginas de fundamentos". La ruta de la página de tokens redirige a la página de color y tokens, de modo que los enlaces existentes sigan funcionando.
